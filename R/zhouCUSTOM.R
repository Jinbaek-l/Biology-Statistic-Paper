#' @description Adapted from skedastic::zhou_etal (GPL-3)
#' @source https://github.com/tjfarrar/skedastic/blob/master/R/zhou_etal.R

#' @details  Only one-sided test supported. So it customized to test two-sided.

zhouCUSTOM <- function(mainlm, auxdesign = NA,
                      method = c("pooled", "covariate-specific", "hybrid"),
                      Bperturbed = 500L, seed = 1234, statonly = FALSE, twosided = TRUE) {
  ...
  if (method == "pooled" || method == "hybrid") {
    ...
    if (twosided) { ## Code modified!
      pvalpool <- sum(abs(Wpoolstar) >= abs(Wpoolobs)) / Bperturbed
    } else {
